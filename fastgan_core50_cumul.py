    ...
    for i, train_batch in enumerate(dataset):
        ...
        if (i < start_batch):
            ...
            if i==0:
                ...
                new_x, new_y = train_batch
                new_x = preprocess_imgs(new_x, norm=False, symmetric = False)

                new_x = torch.from_numpy(new_x).type(torch.FloatTensor)
                new_y = torch.from_numpy(new_y).type(torch.LongTensor)
        ...
        for c in range(n_class):
            if training_examples == None:
                training_examples = train_x[train_y.numpy() == c][:2*n_imag]
            else:
                training_examples = torch.cat((training_examples, train_x[train_y.numpy() == c][:2*n_imag]))

        vutils.save_image(training_examples, saved_image_folder+'/training'+'/%d_0.jpg'%i, nrow=2*n_imag)
        # writer.add_image("Training images", vutils.make_grid(training_examples, nrow=2*n_imag, padding=2, normalize=True).cpu())
        # writer.close()
...
if __name__ == "__main__":
    ...
    parser.add_argument('--same_noise', type=bool, default=False, help='apply same noise to G and D with no gradient accumulation')
