...
model = MyMobilenetV1(pretrained=True, latent_layer_num=latent_layer_num, num_classes=n_class)
gen = generator(nz)
...
eval_label = np.random.randint(0, n_class, batch_size)
eval_onehot = np.zeros((batch_size, n_class))
eval_onehot[np.arange(batch_size), eval_label] = 1
eval_noise_[np.arange(batch_size), :n_class] = eval_onehot[np.arange(batch_size)]
...
for ep in range(num_epochs):
    ...
    for i, data in enumerate(train_dataloader):
        ...
        logits, source = model(train_x, latent_input=None, return_lat_acts=False)
        ...
        loss = criterion(logits, train_y) + criterion_source(source, real_label)
